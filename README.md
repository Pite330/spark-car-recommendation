...
一个已跑通的消费者购车推荐和参数分析平台：PySpark 离线清洗公开车型快照，确定性算法按预算和偏好筛选评分；完整参数管道进一步使用 Spearman、Elastic Net 和随机森林分析配置与同月销量的关联。大模型只负责可选文字润色，关闭后主链路不受影响。

## 已完成功能

- 16888（车主之家）完整离线数据，经 Spark 生成 1573 个有效车系、1744 条车系能源推荐记录；
- PySpark 4.0.3 清洗、枚举映射、销量热度归一化、衍生场景标签、CSV/Parquet 输出；
- 输入预算、车身、能源、品牌、场景、座位数和最低月销量；
- 硬性筛选、权重归一化评分、稳定排序和显式条件放宽；
- 3—5 款推荐卡片、匹配分、月销量和至少两条数据依据；
- 2—3 款车型参数表，以及参考起售价与同期月销量绝对值对比；
- Spark 车型库能源/车身分布图；
- OpenAI 兼容说明接口、超时处理和模板化降级；
- 健康检查、推荐和对比接口；
- 1620 个车系、19482 个配置款和 364 万条完整配置参数长表；
- 187 项参数质量、Elastic Net 系数和随机森林重要度；
- 数据依据工作台、全量参数筛选、参数贡献与趋势数据可用性提示；
- 分析概览接口以及共 31 项自动化测试。
...
src/web/                  Flask、推荐页面、数据依据、ECharts 和 LLM 适配
tests/                    算法、接口、降级和产物测试
docs/                     MVP、契约、设计和验收记录
```
